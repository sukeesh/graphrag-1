...
def extract_and_save_table_text(url, csv_file_path):
    ...
    notification_user = parsed_url.path.split('/')[-1].split('.')[0]
    notification_id = query_params.get('id', [None])[0]
    
    if not notification_id:
        print(f"Notification ID not found in URL: {url}")
        return
...
def scrape_range(start_id, end_id, csv_file_path):
    # Base URL format
    base_url = "https://www.rbi.org.in/Scripts/BS_ViewMasDirections.aspx?id={}"
...
def main():
    csv_file_path = "master_directions_metadata.csv"
